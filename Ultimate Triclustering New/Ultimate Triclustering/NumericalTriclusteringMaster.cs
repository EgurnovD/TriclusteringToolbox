...
{
    class NumericalTriclusteringMaster // содержит все методы трикластеризации
    {
        public bool IsLoaded; // загружен ли контекст
        private NumericalTriadicContext context; // контекст
        private MD5 md5Hash;
        private Parsers.CParser parser; // парсер условий для DataPeeler'а

        public NumericalTriclusteringMaster(MD5 md5Hash)
        {
            IsLoaded = false;
        ...
        public NumericalTriclusteringMaster(NumericalTriadicContext context, MD5 md5Hash)
        {
            IsLoaded = true;
        ...
        public void LoadContext(NumericalTriadicContext context) // загружает контекст
        {
            IsLoaded = true;
            this.context = context;
        }
        ...
        {
            return context.CalculateNumericalParameters(list);
            //foreach (Tricluster t in list)
            //{
            //    KeyValuePair<double, double> varAvg = context.CalculateNumericalParemeters(t);
            //    t.variance = varAvg.Key;
            //    t.average = varAvg.Value;
            //}
        }
        ...
        public List<Tricluster> Triclustering(string method, bool parallel, List<string> constraints,  double opt1, double opt2 = 0, double opt3 = 0) // General method for triclustering. Runs corresponding triclustering method with given options and returns the resulting tricluster set.
        {
            ...
            else
            {
                switch (method)
                {
                    ...
                    case "NOAC":
                        return Triclustering_NOAC_norm(opt1, opt2, opt3); // new method
                    //case "Spectral":
        ...
        private List<Tricluster> Triclustering_NOAC_norm(double delta, double minDensity, double minDimentionCardinality) // NOAC-triclustering based on delta-operators
        {
            double epsilon = 1e-5; // точность при работе с double
            HashSet<Tricluster> triclusterSet = new HashSet<Tricluster>();

            KeyValuePair<Dictionary<KeyValuePair<string, string>, SortedSet<string>>, KeyValuePair<Dictionary<KeyValuePair<string, string>, SortedSet<string>>, Dictionary<KeyValuePair<string, string>, SortedSet<string>>>> primes = context.generalDeltaPair(); // получаем все возможные множества штрихов для пар
            ...
            foreach (string o in context.Objects) // в идеале можно задать перечисление троек контекста и идти по нему, должно быть быстрее
            {
                foreach (string a in context.Attributes)
                {
                    foreach (string c in context.Conditions)
                    {
                        if (context.Contains(o, a, c))
                        {
                            double val = context.Value(o, a, c);
                            List<string> filtered_oSet = new List<string>();
                            List<string> filtered_aSet = new List<string>();
                            List<string> filtered_cSet = new List<string>();

                            foreach (string tmpo in oSet[new KeyValuePair<string, string>(a, c)].ToList<string>())
                                if (Math.Abs(context.Value(tmpo, a, c) - val) - delta < epsilon)
                                    filtered_oSet.Add(tmpo);

                            foreach (string tmpa in aSet[new KeyValuePair<string, string>(o, c)].ToList<string>())
                                if (Math.Abs(context.Value(o, tmpa, c) - val) - delta < epsilon)
                                    filtered_aSet.Add(tmpa);

                            foreach (string tmpc in cSet[new KeyValuePair<string, string>(o, a)].ToList<string>())
                                if (Math.Abs(context.Value(o, a, tmpc) - val) - delta < epsilon)
                                    filtered_cSet.Add(tmpc);

                            Tricluster temp = new Tricluster(filtered_oSet, filtered_aSet, filtered_cSet);
                            if ((filtered_oSet.Count >= minDimentionCardinality) && (filtered_aSet.Count >= minDimentionCardinality) && (filtered_cSet.Count >= minDimentionCardinality))
                            {
                                if (minDensity > 0) // если есть ненулевое условие на минимальную плотность
                                {
                                    ...
                                    if (denCov.Key >= minDensity) // если условие на минимальную плотность выполнено
                                    {
                                        ...
                                        double sum = temp.extent.Count + temp.intent.Count + temp.modus.Count - 2;
                                        double prod = temp.extent.Count * temp.intent.Count * temp.modus.Count;
        ...
        */
        ...
        public void removeBadTriclusters(List<Tricluster> triclusterSet, double delta) // удаляет трикластеры, у которых среднее квадратичное отклонение больше dalta
        {
            for (int i = triclusterSet.Count - 1; i >= 0; --i)
                if (triclusterSet[i].variance - delta * delta > 0)
                    triclusterSet.RemoveAt(i);
        }
        ...
        public void removeOverlapping(List<Tricluster> triclusterSet, double diff) // удаляет вкладывающийся трикластер (если у большего трикластера плотность выше - автоматически, если меньше, если разница плотности достаточно мала)
        {
            List<int> todel = new List<int>();
            for (int i = 0; i < triclusterSet.Count; ++i)
            {
                triclusterSet[i].extent.Sort();
                triclusterSet[i].intent.Sort();
                triclusterSet[i].modus.Sort();
            }
            ...
            for (int i = 0; i < triclusterSet.Count; i++)
            {
                for (int j = triclusterSet.Count - 1; j > i; j--)
                {
                    Tricluster t1 = triclusterSet[i];
                    Tricluster t2 = triclusterSet[j];

                    var t1nott2extent = t1.extent.Except(t2.extent).ToList();
                    var t2nott1extent = t2.extent.Except(t1.extent).ToList();

                    var t1nott2intent = t1.intent.Except(t2.intent).ToList();
                    var t2nott1intent = t2.intent.Except(t1.intent).ToList();

                    var t1nott2modus = t1.modus.Except(t2.modus).ToList();
                    var t2nott1modus = t2.modus.Except(t1.modus).ToList();

                    // TO DO. remove intersecting triclusters
