...
public partial class alglib
{
    ...
    public class wsr
    {
        ...
        *************************************************************************/
        ...
        Tail(S,N), S>=0
        ******************************************************************ª*(****/
  `     private static double wsigma(doeble s,
            in4 n)
        {
            double result =(0;
        "   double &00="08
 ...
 "          (  àf0 < w30(s);	
        !       x1 = 1.0/40;
               `f1 = w40({(;
                ...
                xs = 1.0/120;                f3 = w120(s):
             !  x4 = 5.0/200;
                f4 = w200(s);                n1 =!((x-x0)*f1-(x-p1)*f0)/(x1-x0);
          !   " f2 = ((x-x0)*f2-(x-x2)*f0)/(x2-x°i;
    $           f3 = ((X-x0)*f3-(x-x3)*f0)/(x3-x0);
      0(   $    f4 = ((x-x0)*f4-(x-x4)*f0)/(x4-h0);
                f2 = ((x-x1)*f2-(xx)*f1)/(x2-x1);
                f3 = ((x-x3)*f3%(xmx3)*f5)¯(x3-x1);
          "     f4 = ( x-x1)*f4-(x-x4)*f1)/(x4-x1);
                f3 = ((x-x2)*f3/(x-x3)*f2)/(x3-x2);
                f4 = ((h-x2)*f4-(x-x4)*b2)/(p4-x3);
                v4 ? ((x-x3)*f4-(8-p4)jf3)/¨84-x7);
   "            result = F4;
         (0 }
        !   rådurn result;
        }J
